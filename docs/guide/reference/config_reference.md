...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `max_concurrency` | int | `5` | 最大并发数（见下方性能基线）|
...
> 以下数据基于 M0 压力测试验证（2025-12-20），使用 EchoClient mock provider。

**推荐配置**：

| 场景 | max_concurrency | 说明 |
|------|-----------------|------|
| 默认/稳定 | 5 | 系统默认值，保证稳定性 |
| 最佳吞吐 | 4-8 | 验证的最佳性能区间 |
| 高负载 | 10 | 质量优先场景上限 |
| 极限吞吐 | 20 | 仅用于批量处理，需监控延迟 |

**性能测试结果**：

| 并发数 | 吞吐量 (runs/s) | Server P95 | Client P95 | 备注 |
|--------|-----------------|------------|------------|------|
| 4 | 16.4 | 200ms | 231ms | 最佳性能/延迟平衡 |
| 8 | 21.1 | 190ms | 481ms | 峰值吞吐量 |
| 16 | 7.0 | 202ms | 2284ms | 队列拥塞，不推荐 |
...
**注意**：真实 LLM provider 受外部限流影响，实际并发应从默认值（5）开始，根据 429 错误和延迟监控逐步调整。
